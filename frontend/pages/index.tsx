...
export default function Home() {
  ...
  useEffect(() => {
    api.get('/products?limit=6').then(r => setFeatured(r.data.products)).catch(() => {});
    // Load categories
  ...
  return (
    <div>
      ...
      {/* Hero Section */}
      <section className={`relative w-full h-[600px] md:h-[700px] bg-gradient-to-br from-${colors.bgLight} via-${colors.bg} to-${colors.bgLight} overflow-hidden`}>
        ...
        <div className="absolute inset-0 opacity-20">
          <div className={`w-full h-full bg-gradient-to-r from-${colors.accent} via-${colors.secondary} to-${colors.accent}`}></div>
        </div>
      </section>

      {/* Featured Products Section */}
      <section className={`py-16 px-6 bg-gradient-to-b from-white to-${colors.bg}`}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className={`text-4xl md:text-5xl font-bold text-${colors.text} mb-4`}>Öne Çıkan Ürünler</h2>
            <p className={`text-lg text-${colors.primaryDark} font-semibold`}>En popüler pijama takımlarımız</p>
          </div>
      ...
      {/* Additional Section */}
      <section className={`py-16 px-6 bg-gradient-to-br from-${colors.bgLight} via-${colors.bg} to-${colors.bgLight}`}>
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div className={`bg-white/80 p-6 rounded-2xl shadow-lg border-2 border-${colors.border}`}>
